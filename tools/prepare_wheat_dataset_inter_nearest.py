...
parser = argparse.ArgumentParser(description='Prepare dataset for training with remapped rois with inter nearest interpolation')
...
mapxs = np.load('mapxs.npy')
mapys = np.load('mapys.npy')

rois_map = {k: v for k, v in zip(rects, rois)}          # make a lookup table
remap_x_map = {k: v for k, v in zip(rects, mapxs)}
remap_y_map = {k: v for k, v in zip(rects, mapys)}
...
for filename in glob.glob(os.path.join(args.framesdataset, '*.jpg')):
    ...
    for crop, label in crop_label_data.items():
        roi = rois_map[crop]
        mapx = remap_x_map[crop]
        mapy = remap_y_map[crop]
        x, y, w, h = roi_to_rect(roi)
        img_crop = img[y:y+h, x:x+w]
        img_crop = cv2.remap(img_crop, mapx, mapy, cv2.INTER_NEAREST)
        crop_coords = "_".join([f'{v}' for v in [x, y, w, h]])
        dirname = os.path.join(args.output, label)
        if not os.path.exists(dirname):
            os.makedirs(dirname)
        dst_filename = os.path.join(dirname, fileid + '_' + crop_coords + '.jpg')
